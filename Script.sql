...
CREATE OR REPLACE FUNCTION obtener_usuario_por_credenciales(
    ...
    genero_persona VARCHAR(10),
    edad_persona INT,
    direccion_persona VARCHAR(255)
) AS $$
BEGIN
    RETURN QUERY
    SELECT u.userId, u.email, u.username, u.password,
           p.cedula, p.nombre, p.genero, p.edad, p.direccion
    FROM Usuario u
    INNER JOIN Persona p ON u.personaCedula = p.cedula
    WHERE (u.email = login_param OR u.username = login_param)
        AND u.password = password_param;
END;
$$ LANGUAGE plpgsql;


SELECT * FROM obtener_usuario_por_credenciales('juanperez35', 'claveSegura1');


CREATE OR REPLACE FUNCTION listar_plataformas()
RETURNS TABLE (
    plataformaId VARCHAR(50),
    nombre VARCHAR(100),
    tipo VARCHAR(50),
...
BEGIN
    RETURN QUERY
    SELECT p.plataformaId, p.nombre, p.tipo, p.usuariosConectados
    FROM Plataforma p;
END;
$$ LANGUAGE plpgsql;

SELECT * FROM listar_plataformas();
